...
def producetableswithmissingentries(imputedic,numtables,nummissingentries, picknewposandremovefunc):
    ...
    for entry in imputedic:
        if type(imputedic[entry])==dict:
            ...
            numrow=len(imputedic[entry]["originaltable"])-1#adjust for fact that first entry in row or col must be a str name            
            numcol=len(imputedic[entry]["originaltable"][0])-1
